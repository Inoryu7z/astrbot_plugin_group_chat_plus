...
【话题耗尽】
- 循环附和：A表达→B附和→A换表述→B再附和，此类无实质推进的循环对话必须返回no
- 当前消息仅为认同性回复（未提供新信息或新角度），且你已回复过该话题，返回no
...
- 用户明确拒绝或反感（"别烦""闭嘴""滚"等）
- 连续对话模式显示发送者一直在跟别人对话
- 冷却信号：用户已转向与他人对话（@他人、与特定用户连续交流）

【空@与关键词触发】
- 空@：发送者仅@你而未附文字，先判断意图。对方可能只是召唤你，未必要求你接前面某条消息；不确定时简单回应即可，不强接上下文
- 关键词触发：消息因含关键词被筛选，不代表必须回复。仍需判断消息是否实际发给你、是否需要你介入
