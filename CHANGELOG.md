...
🔧 **问题1：群聊 bot 无法感知时间流逝**
- **现象**: bot 前天说"正在和a比赛"，两天后用户问"你在做什么"，bot 仍说在准备比赛。私聊无此问题
- **根因**: `format_context_for_ai` 中 bot 自己的历史回复被 `not is_bot` 条件过滤，不带时间戳，AI 无法知道自己说过的话是多久之前
- **修复**: 移除 `not is_bot` 限制，改为时间差阈值策略——仅当消息与当前时间差超过阈值（默认30分钟）时才显示时间戳。连续对话无污染，跨时段对话自动显示，让 AI 感知时间流逝
- **新增配置**: `timestamp_threshold_minutes`（默认30，设为0表示始终显示）

🔧 **问题2：分段消息丢失（空@bot 看不到前一条内容消息）**
- **现象**: 用户先发"为什么你要这么说话"，紧接着发"@bot"，bot 回复"突然艾特我又不说话"，完全忽略第一条消息
- **根因**: 内容消息的缓存写入在 AI 决策之后（6-8秒），而空@消息的上下文构建在处理早期（立即），两者并发导致空@消息读不到前一条消息
- **修复**: 新增 `recent_processing_messages` 临时存储，在消息进入处理时立即写入内容，空@消息的上下文构建可立即读取，无需等待。处理完成后保留60秒再清理，确保后续空@消息有机会读到

**修改文件**: main.py, utils/context_manager.py, utils/proactive_chat_manager.py, _conf_schema.json, metadata.yaml
...
**默认提示词模板优化，解决重复回复、旧话题追击、工具调用失控三大问题**

**读空气提示词（decision_ai.py - SYSTEM_DECISION_PROMPT）**:
- **判断原则调整**  倾向于积极参与  像真人一样选择性参与，降低无意义回复的通过率
- **新增【话题耗尽检测】**  检测同义循环、附和型消息，话题已充分讨论时返回no，避免对同一观点反复附和
- **【防止重复】增强**  从只查自己的回复扩展到检查2轮内同话题回复，连续回复后需新角度才考虑再回复
- **输出要求保持原版**  你的目标是促进对话，不确定时倾向于回复（通过率问题由话题耗尽检测和防止重复控制，不靠改默认立场）

**回复提示词（reply_handler.py - SYSTEM_REPLY_PROMPT）**:
- **新增【话题锚定】**  回复应与当前消息话题相关，不从历史或记忆中拾取无关话题；被批评时不转移注意力
- **新增【同话题换表述】**  同话题再次回复时必须换切入角度，找不到新角度则用极简回应
- **新增【工具调用】独立段落**  替换原根据需要调用可用工具，改为通用原则：仅在对方消息包含对工具功能的明确请求时才调用，不确定时不调用

**同发送者串行决策（代码改动）**:
- 新增配置项 enable_sender_serial_decision，同一发送者连发多条消息时串行判断，前一条判断yes时注入话题判断提示，避免同话题重复回复
- 涉及文件：main.py、decision_ai.py、_conf_schema.json

**修改文件**:
- utils/decision_ai.py  默认提示词模板修改 + 串行决策参数
- utils/reply_handler.py  默认提示词模板修改
- main.py  串行决策逻辑
